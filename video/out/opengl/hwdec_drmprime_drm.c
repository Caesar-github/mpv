    ...
    if (hw_image) {
        ...
        if (desc) {
            ...
            if (request) {
                ...
                drm_object_set_property(request, p->ctx->drmprime_video_plane, "CRTC_X",  MP_ALIGN_DOWN(dx, 2));
                drm_object_set_property(request, p->ctx->drmprime_video_plane, "CRTC_Y",  MP_ALIGN_DOWN(dy, 2));
